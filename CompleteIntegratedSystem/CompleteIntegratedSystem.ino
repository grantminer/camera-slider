...
void prepSteppers() {
  wakeUp(); // Turn on steppers
  
  speedCounterTranslate = sliderValue_store1;
  positionCounterTranslate = sliderValue_store2*10; // convert speed to mm/s

  speedCounterPan = sliderValue_store3;
  positionCounterPan = sliderValue_store4;

  // Guard against unacceptable values
  ...
  roundPanValues(); // Round to the tenths place

  //set speeds of travel
  ...
  // Activate camera shutter/ begin capture
...
void runSteppers() {
  ...
  if ((!translateStepper.motionComplete()) 
       || (!panStepper.motionComplete())) {

    // move one step for each stepper
    translateStepper.processMovement();
    panStepper.processMovement();

    // Define the steppers as not homed. 
    homed = false;
